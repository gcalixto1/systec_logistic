...
<div class="container-fluid">
    <div class="card-header">
        <h4 class="card-title text-black"><i class="fa fa-truck"></i> Gestion de UMB</h4>
    </div>
    <br>
    <div class="col-lg-12">
        ...
        <table class="table table-responsive" id="borrower-list">
            ...
            <tbody>
                <?php
                $i = 1;
                $qry = $conexion->query("SELECT * FROM relaciones");
...
<script>
    ...
    $('#new_categoria').click(function () {
        uni_modal("Gestion de Relaciones", "manage_relacion.php")
    })
    $('#borrower-list').on('click', '.edit_borrower', function () {
        uni_modal("Modificar Relacion", "manage_relacion.php?id=" + $(this).attr('data-id'))
    })
    $('#borrower-list').on('click', '.delete_borrower', function () {
        _conf("Esta seguro que quiere eliminar esta relacion?", "delete_borrower", [$(this).attr('data-id')])
    })

    function delete_borrower($id) {
        start_load()
        $.ajax({
            url: 'ajax.php?action=delete_relacion',
            method: 'POST',
            data: {
                id: $id
            },
