...
if ($id > 0) {
    $query = $conexion->query("SELECT * FROM tipo_producto WHERE id = $id");
            ...
            <div class="form-body">
                <div class="card-body">
                    ...
                    <div class="form-group col-md-12">
                            ...
                            <input type="text" name="tipo" id="tipo" class="form-control" placeholder="Ingrese un Tipo"
                              value="<?php echo isset($meta['tipo']) ? htmlspecialchars($meta['tipo']) : ''; ?>" required>  
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    $('#saveapertura').submit(function (e) {
        ...
        var categoriaDes = $('#tipo').val().trim();
        ...
        start_load();
        $.ajax({
            url: 'ajax.php?action=save_tipos',
            method: 'POST',
            data: $(this).serialize(),
