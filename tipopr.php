...
<div class="container-fluid">
    <div class="card-header">
        <h4 class="card-title text-black"><i class="fa fa-truck"></i> Gestion de Tipos de Producto</h4>
    </div>
    <br>
    <div class="col-lg-12">

        <div class="col-sm-12 col-xs-12 text-right">
            <button class="btn btn-success btn-lg" type="button" id="new_categoria"><i class="fa fa-plus"></i> Nuevo
                Tipos de producto</button>
        </div>
        <br />
        ...
        <table class="table table-responsive" id="borrower-list">
            ...
            <tbody>
                <?php
                $i = 1;
                $qry = $conexion->query("SELECT * FROM tipo_producto");
...
<script>
    ...
    $('#new_categoria').click(function () {
        uni_modal("Gestion de Tipo de Productos", "manage_tipospr.php")
    })
    $('#borrower-list').on('click', '.edit_borrower', function () {
        uni_modal("Modificar Etiqueta", "manage_tipospr.php?id=" + $(this).attr('data-id'))
    })
    $('#borrower-list').on('click', '.delete_borrower', function () {
        _conf("Esta seguro que quiere eliminar este tipo de producto?", "delete_borrower", [$(this).attr('data-id')])
    })

    function delete_borrower($id) {
        start_load()
        $.ajax({
            url: 'ajax.php?action=delete_tipos',
            method: 'POST',
            data: {
                id: $id
            },
